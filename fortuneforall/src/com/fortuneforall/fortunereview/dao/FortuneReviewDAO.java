...
public class FortuneReviewDAO {
	...
	public List<FortuneReview> selectBoard(){
		...
		try {
			...
			sql.append("select m.member_id, b.reg_Date, c.content from tb97_member m inner join tb97_board b on m.member_no = b.member_no inner join tb97_comment c on b.board_no = c.board_no;");
	...
	public FortuneReview selectBoardByNo(int no) {
		...
		try {
			con = ConnectionPool.getConnection();
			StringBuffer sql = new StringBuffer();
			sql.append("select board_no, title, writer, content, reg_date, file from tb97_board where = '"+no );
	...
	public void insertBoard(FortuneReview fortuneReivew) {
		...
		try {
			con = ConnectionPool.getConnection();
			StringBuffer sql = new StringBuffer();
			sql.append("insert into tb97_board(board_no, title, writer, content, table_type) valuse(s_board_no_nextval, ?, ?, ?, '4')");
